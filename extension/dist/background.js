...
async function handleStartRecording(params) {
  ...
  if (!tabId) {
    return {
      success: false,
      error: "No connected tab found for recording. Click the RunBrowser extension icon on the tab you want to record."
    };
  }
      ...
      chrome.tabCapture.getMediaStreamId({ targetTabId: tabId }, (id) => {
        if (chrome.runtime.lastError) {
          ...
          if (errorMsg.includes("Extension has not been invoked") || errorMsg.includes("activeTab")) {
            reject(new Error(`${errorMsg}. Click the RunBrowser extension icon on this tab to enable recording.`));
          } else {
...
const RELAY_PORT = Number("19988") || 19988;
...
function hasOptionalSessionId(params) {
  return params.sessionId === void 0 || typeof params.sessionId === "string";
}
function isRelayToExtensionMessage(value) {
  ...
  switch (value.method) {
    ...
    case "forwardCDPCommand": {
      if (!hasNumericId(value) || !isRecord(value.params) || typeof value.params.method !== "string") {
        return false;
      }
      return hasOptionalSessionId(value.params);
    }
    case "startRecording":
    case "stopRecording":
    case "isRecording":
...
async function detectBrowserName() {
  ...
  const brands = (_a = navigatorWithUaData.userAgentData) == null ? void 0 : _a.brands;
...
class ConnectionManager {
  ...
  async ensureConnection() {
    ...
    if (store.getState().connectionState === "extension-replaced") {
      throw new Error("Another RunBrowser extension is already connected");
    }
  ...
  async connect() {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        await fetch(`http://${RELAY_HOST}:${RELAY_PORT}`, { method: "HEAD", signal: AbortSignal.timeout(2e3) });
    ...
    {
      relayUrl.searchParams.set("v", "0.0.7");
    }
    ...
    this.ws.onmessage = async (event) => {
      let parsedMessage;
      try {
        const rawMessage = typeof event.data === "string" ? event.data : event.data instanceof Blob ? await event.data.text() : String(event.data);
        parsedMessage = JSON.parse(rawMessage);
      } catch (error) {
        const message2 = getErrorMessage(error);
        logger.debug("Error parsing message:", error);
        sendMessage({ error: { code: -32700, message: `Error parsing message: ${message2}` } });
        return;
      }
      if (!isRelayToExtensionMessage(parsedMessage)) {
        logger.debug("Rejected invalid relay message payload");
        sendMessage({ error: { code: -32600, message: "Invalid request payload" } });
        return;
      }
      ...
      if (message.method === "createInitialTab") {
        try {
          ...
          sendMessage({ id: message.id, error: errorMessage });
        }
        return;
      }
  ...
  handleClose(reason, code) {
    ...
    if (isExtensionReplaced) {
      logger.debug("Disconnected: another RunBrowser extension connected (this one was idle)");
      store.setState({
        tabs: /* @__PURE__ */ new Map(),
        connectionState: "extension-replaced",
        errorText: "Another RunBrowser extension took over the connection"
      });
      return;
    }
    if (isExtensionInUse) {
      logger.debug("Rejected: another RunBrowser extension is actively in use");
      store.setState({
        tabs: /* @__PURE__ */ new Map(),
        connectionState: "extension-replaced",
        errorText: "Another RunBrowser extension is actively in use"
      });
      return;
    }
    store.setState((state) => {
...
function sendMessage(message) {
  ...
  if (((_a = connectionManager.ws) == null ? void 0 : _a.readyState) === WebSocket.OPEN) {
    try {
      ...
      const messageType = typeof message === "object" && message !== null && "method" in message ? message.method : void 0;
      console.debug("ERROR sending message:", error, "message type:", messageType || "response");
    }
  }
}
async function syncTabGroup() {
  ...
  try {
    ...
    const existingGroups = await chrome.tabGroups.query({ title: "runbrowser" });
    if (connectedTabIds.length === 0) {
      for (const group of existingGroups) {
        ...
        logger.debug("Cleared runbrowser group:", group.id);
      }
      return;
    }
    ...
    if (existingGroups.length > 1) {
      ...
      for (const group of duplicates) {
        ...
        logger.debug("Removed duplicate runbrowser group:", group.id);
      }
    }
    ...
    if (tabsToAdd.length > 0) {
      if (groupId === void 0) {
        ...
        await chrome.tabGroups.update(newGroupId, { title: "runbrowser", color: "green" });
  ...
  switch (msg.params.method) {
    case "Runtime.enable": {
      ...
      const runtimeSession = {
        ...debuggee,
        sessionId: msg.params.sessionId !== (targetTab == null ? void 0 : targetTab.sessionId) ? msg.params.sessionId : void 0
      };
      try {
  ...
  try {
    ...
    const contextMenuScript = `
      document.addEventListener('contextmenu', (e) => {
        window.__runbrowser_lastRightClicked = e.target;
      }, true);
    `;
...
async function connectTab(tabId) {
  try {
    ...
    const isExtensionInUse = error.message === "Extension Already In Use" || error.message === "Another RunBrowser extension is already connected";
...
const icons = {
  ...
  extensionReplaced: {
    ...
    title: "Another RunBrowser extension connected - Click to retry",
    badgeText: "!",
...
chrome.contextMenus.remove("runbrowser-pin-element").catch(() => {
}).finally(() => {
  chrome.contextMenus.create({
    id: "runbrowser-pin-element",
    title: "Copy RunBrowser Element Reference",
    contexts: ["all"],
    visible: false
  });
});
function updateContextMenuVisibility() {
  ...
  chrome.contextMenus.update("runbrowser-pin-element", { visible: isConnected });
}
    ...
    tabGroupQueue = tabGroupQueue.then(async () => {
      ...
      if (changeInfo.groupId === groupId) {
        if (!tabs.has(tabId) && !isRestrictedUrl(tab.url)) {
          logger.debug("Tab manually added to runbrowser group:", tabId);
        ...
        logger.debug("Tab manually removed from runbrowser group:", tabId);
        await disconnectTab(tabId);
      }
    }).catch((e) => {
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId !== "runbrowser-pin-element" || !(tab == null ? void 0 : tab.id)) return;
  ...
  const name = `runbrowserPinnedElem${count}`;
  ...
  try {
    const result = await chrome.debugger.sendCommand(debuggee, "Runtime.evaluate", {
      expression: `
        if (window.__runbrowser_lastRightClicked) {
          window.${name} = window.__runbrowser_lastRightClicked;
          '${name}';
        } else {
